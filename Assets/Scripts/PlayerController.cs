...
public class PlayerController : MonoBehaviour
{
    private Rigidbody rb; // stores the cube's Rigidbody
    private float movementX; // left/right input
    private float movementY; // forward/back input
    public float speed = 10f; // movement force
    public float rotateSpeed = 120f; // how fast the cube rotates

    void Start()
    {
        rb = GetComponent<Rigidbody>();

        // Prevent cube from tipping or rolling like a ball
        rb.freezeRotation = true;
    }

    void OnMove(InputValue movementValue)
    {
        // Read input (x = left/right, y = forward/back)
    ...
    void FixedUpdate()
    {
        ...
        Vector3 moveDirection = transform.forward * movementY;

        // Apply movement force
        rb.AddForce(moveDirection * speed);

        // Left/right rotation (turning the cube, not tipping it)
        if (movementX != 0)
        {
            float turn = movementX * rotateSpeed * Time.fixedDeltaTime;
